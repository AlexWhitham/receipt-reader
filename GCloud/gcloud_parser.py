...
class GcloudParser:
    def __init__(self, debug=False, min_length=5, max_height=1):
        ...
        self.allowed_labels = ['article', 'price',
                               'market', 'address', 'date', 'misc']
    ...
    def parse_response(self, gcloud_response):
        ...
        seen_prices = []
        parsed_y = 0
        base_ann = gcloud_response.text_annotations[0].encode('utf-8')
            ...
            if t_type == 'text':
                ...
                for j, p_ann in enumerate(sorted_annotations):
                    ...
                    if skip_this:
                        continue
                    p_xmin = np.min(
                        [v.x for v in p_ann.bounding_poly.vertices])
                    p_xmax = np.max(
                        [v.x for v in p_ann.bounding_poly.vertices])
                    p_ymin = np.min(
                        [v.y for v in p_ann.bounding_poly.vertices])
                    p_ymax = np.max(
                        [v.y for v in p_ann.bounding_poly.vertices])
                    if p_ymax < ymin or p_ymin > ymax:
                        continue
                    line_overlap = np.min(
                        [p_ymax-ymin, ymax-p_ymin]) / np.max([p_ymax-p_ymin, ymax-ymin])
                    if line_overlap < 0.5:
                        continue
                    if is_hanging:
